...
def parse_OFCB_trade_session(logs_path, current_year, current_month, current_day):
    ''' OFCB FIX Trade session parser '''
    ...
    test2['Time'], test2['in_out'], test2['id'], test2['exec_price'], test2['exec_vol'], test2['instr'], test2['side'] = np.vectorize(extract_params)(test2[0], current_year, current_month, current_day)
    ...
    if out.empty==False:
        ...
        for i in set(out.id.tolist()):
            ...
            times.append(float(out.at[i, 'exec_vol']))
            times.append(float(out.at[i, 'exec_price']))
        ...
        Time_array.columns = ['Time', 'Instrument', 'req_vol', 'req_price', 'exec_time', 'exec_price', 'exec_vol', 'side']
        Time_array['exec_time'] = Time_array['exec_time'].apply(lambda x: datetime.strptime(x, '%Y-%m-%d %H:%M:%S.%f'))
        Time_array['Time'] = Time_array['Time'].apply(lambda x: datetime.strptime(x, '%Y-%m-%d %H:%M:%S.%f'))  
        ...
        Time_array['improvement'] = np.vectorize(price_improvement)(Time_array['req_price'], Time_array['exec_price'], Time_array['side'])
        Time_array['full_time_diff'] = (Time_array['exec_time'] - Time_array['Time']).apply(lambda x: (x/np.timedelta64(1,'s'))*1000)
        Time_array['exec_%'] = Time_array['exec_vol']/Time_array['req_vol']
        Time_array['LP']='OFCB'
        
        return Time_array, ins[['Time']]
    else:
        del out[0], out['in_out']
        return out, out        

        
def generate_OFCB_execution_report(output_name, start_date, end_date):
    ''' Parsing '''
    ...
    for i in date_generated:
        ...
        if logs_path in glob('L:\\Alfa\\'+date+'\\*'):           
            new_data, unusual_rejects = parse_OFCB_trade_session(logs_path, current_year, current_month, current_day)
