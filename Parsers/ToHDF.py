...
output = 'F:\\DataBase\\DealsFrom27Apr.h5'
...
def parse_day_of_deals(date, detalization_level):
    ...
    if not day_deals.empty:
        store.append('Deals', day_deals[client_only_non_mixed_types], data_columns = True, complevel = 9, complib='zlib',min_itemsize = {'values': 100})
        for i, row in day_deals.iterrows():
            base_path = str(row['AggrId'])
            sdt_methods_execution = row['SdtMethodsExecution']
            store.append(base_path+'/SdtMethodsExecution', sdt_methods_execution, min_itemsize = {'values': 100}, complevel = 9, complib='zlib')
            bid_quotes = row['BidQuotes']
            store.append(base_path+'/BidQuotes', bid_quotes, min_itemsize = {'values': 100}, complevel = 9, complib='zlib')
            ask_quotes = row['AskQuotes']
            store.append(base_path+'/AskQuotes', ask_quotes, min_itemsize = {'values': 100}, complevel = 9, complib='zlib')
    # print(store)
    # store.close()
...
def get_existing_deals_ids(store):
    ...
    return list(set(aggr_ids_cleaned))

if __name__ == "__main__":
    start_date = datetime(2015, 7, 13)
    end_date = datetime(2015, 7, 18)
