...
client_only_non_mixed_types = ['OrderType', 'ExactTime', 'ClientName', 'Side', 'AggrId','Instrument', 'ReqLot',
                               ...
                               'Marination', 'UnexpectedMarination', 'Pnl', 'SoftPnl']
...
def parse_day_of_deals(date, detalization_level):
    ...
    for i, row in day_deals.iterrows():
        ...
        store.append(base_path+'/SdtMethodsExecution', sdt_methods_execution, min_itemsize = {'values': 30})
        bid_quotes = row['BidQuotes']
        store.append(base_path+'/BidQuotes', bid_quotes, min_itemsize = {'values': 30})
        ask_quotes = row['AskQuotes']
        store.append(base_path+'/AskQuotes', ask_quotes, min_itemsize = {'values': 30})
    # print(store)
    # store.close()
