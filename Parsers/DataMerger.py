...
def Data_merger(type, pattern):
    ...
    if pattern == ' Nonaggr.csv':
        Deals['Time'] = Deals['Time'].apply(datetime_with_dot)
    return Deals


def datetime_with_dot(time):
    if '.' in time:
...
def comission_array(Deals):
    ...
    Deals = Deals[(Deals['Result'] == 'Ok') & (Deals['Flags'] != 'Swap') & (Deals['Flags'] != 'Swap Counter CCY') & (
    Deals['Flags'] != 'Deleted External') & (Deals['Flags'] != 'Deleted') & (Deals['Flags'] != 'Deleted Counter CCY')]
    ...
    Deals['Comission_per_Deal'] = Deals.apply(
        lambda row: 0.8 if (row['Source'] != 'MCX' and row['Source'] != 'MOEX') else 0, axis=1)

    Deals['FX Aggr Comission'] = Deals.apply(lambda row: row['Amount(USD)'] * 2.75 / 1000000 if (
    row['Trader'] == 'DEALER' and row['Source'] != 'RBRU' and row['Source'] != 'RZBM' and row[
        'Source'] != 'RZBM2') else 0, axis=1)
    Deals['Comission_Int'] = Deals.apply(
        lambda row: row['Amount(USD)'] * 5.7 / 1000000 if (row['Connection Type'] == '.int') else 0, axis=1)
    Deals['Comission_360t'] = Deals.apply(
        lambda row: row['Amount(USD)'] * 12 / 1000000 if (row['Connection Type'] == '360t') else 0, axis=1)
    Deals['Comission_MOEX'] = Deals.apply(
        lambda row: row['Amount(USD)'] * 14 / 1000000 if (row['Source'] == 'MOEX') else 0, axis=1)
    Deals['Comission_EBS'] = Deals.apply(
        lambda row: row['Amount(USD)'] * 20 / 1000000 if (row['Source'] == 'EBS' and row['Trader'] == 'DEALER') else 0,
        axis=1)

    Deals['Total Comission'] = Deals['Comission_per_Deal'] + Deals['FX Aggr Comission'] + Deals['Comission_Int'] + \
                               Deals['Comission_360t'] + Deals['Comission_MOEX'] + Deals['Comission_EBS']
